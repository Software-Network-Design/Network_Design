...
def send_file(file_path, send_socket):
    if os.path.isfile(file_path):
        ...
        send_socket.send((file_name + '|' + file_size).encode('utf-8'))
...
def recv(user_dict, group, my_id):
    while True:
        ...
        if package_type == 3:
            sender = data['send']
            message = data['info']
            user_dict[sender].message_queue.put(message)
            # TODO: 如果正显示对应聊天窗口，显示消息内容
        # 群聊消息
        elif package_type == 4:
            sender = data['send']
            message = data['info']
            group.message_queue.put(message)
            # TODO: 如果正显示群聊窗口，显示消息内容
        # 用户下线
        elif package_type == 5:
...
def init_user_list(user_list, response_dict):
    ...
    stranger_count = response_dict['strangers']['strangers_num']
    friend_count = response_dict['friends']['friends_num']
